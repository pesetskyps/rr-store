...
asda
        ...
        public ActionResult ExternalLogin(string provider, string returnUrl)
        {
            ...
            if (provider == "Plex ADFS")
            {
                ctx.Authentication.Challenge(
                    new AuthenticationProperties
                    {
                        RedirectUri = Url.Action("LoginCallbackPlexAdfs", "Account", new { provider })
                    },
                    provider);
            }
            else if (provider == "Plextest ADFS")
            {
                ctx.Authentication.Challenge(new AuthenticationProperties
                {
                    RedirectUri = Url.Action("LoginCallbackIndiaPlextestAdfs", "Account", new { provider })
                },
                provider);
            }
        ...
        public ActionResult LoginCallbackPlexAdfs(string provider)
        {
            var ctx = Request.GetOwinContext();
        ...
        public ActionResult LoginCallbackIndiaPlextestAdfs(string provider)
        {
            var ctx = Request.GetOwinContext();
            var result = ctx.Authentication.AuthenticateAsync("ExternalCookie").Result;
            ctx.Authentication.SignOut("ExternalCookie");
